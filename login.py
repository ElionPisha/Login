...
class Login:
    ...
    def loginform(self):
        ...
        self.img=ImageTk.PhotoImage(file="hero-bg.jpg")
        ...
        btn2=Button(frame_input, text="Login", command=self.login, cursor='hand2', font=('times new roman', 15,), fg='white', bg='orange', bd=0, width=15, height=1)
        btn2.place(x=90, y=340)

    
        btn3=Button(frame_input, command=self.Register, text="Not Registered", cursor='hand2', font=('calibri', 10,), bg='white', fg='black', bd=0)
    ...
    def Register(self):
        ...
        Frame_login1.place(x=0, y=0, height=700, width=1280)

        self.img=ImageTk.PhotoImage(file="hero-bg.jpg")
        img=Label(Frame_login1, image=self.img) .place(x=0, y=0, width=1100, height=600)
        ...
        self.entry.place(x=330, y=245, width=270, height=35)
        ...
        btn3=Button(frame_input2, command=self.loginform, text="Already Registerd", cursor='hand2', font=('claibri', 10,), fg='black', bg='white', bd=0)
        btn3.place(x=110, y=390)
    ...
    def register(self):
        ...
        if self.entry.get()=='' or self.entry2.get()=="" or self.entry3.get()=="" or self.entry4.get()=="":
            ...
            try:
                con=pymysql.connect(host="localhost", user="root", password='', database="pythongui")
                cur=con.cursor()
                cur.execute("select * from register where email_id=%s", self.entry3.get())
                row=cur.fetchone()
                if row!=None:
    ...
    def appscreen(self):
        ...
        label2=Label(Frame_login, text="Yotube", font=('times new roman', 32, 'bold'), fg="black", bg="white")
        label2.place(x=235, y=160)
        label3=Label(Frame_login, text="Plase Subscribe", font=('times new roman', 32, 'bold'), fg='black', bg='white')
